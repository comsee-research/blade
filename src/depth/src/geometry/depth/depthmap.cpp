...
{
#if 0
	const std::size_t R = static_cast<std::size_t>(std::ceil(mfpc.mia().radius()));
	...
	for (const P3D& p: pc)
	{
		...
		{
			const auto& o = obs[i];
			const std::size_t u_ 	= static_cast<std::size_t>(std::trunc(o.u));
			const std::size_t v_ 	= static_cast<std::size_t>(std::trunc(o.v));
			const std::size_t rho 	= static_cast<std::size_t>(std::ceil(std::fabs(o.rho))); 
			
			//get micro-image
			const auto [k, l] = mfpc.mia().uv2kl(u_, v_);
			const P2D c = mfpc.mia().nodeInWorld(k, l);
			
			//adssign depth for all pixels within blur radius
			for (std::size_t u = u_ - rho; u <= u_ + rho; ++u)
			{	
				for (std::size_t v = v_ - rho; v <= v_ + rho; ++v)
				{
					if (((c - P2D{u, v}).norm() > R) or (P2D{u_, v_} - P2D{u, v}).norm() > rho) continue;
					
					if (state(u, v) == DepthInfo::State::UNINITIALIZED) 
					{
						state(u, v) = DepthInfo::State::INITIALIZED;
						confidence(u, v) = 0.;
						depth(u, v) = 0.;
					}
	...
	for(std::size_t k = 0; k < width(); ++k)
	{
		...
		{
			if (state(k,l) != DepthInfo::State::UNINITIALIZED)
			{			
				depth(k,l) 	= depth(k,l) / confidence(k,l);
			}
			confidence(k,l) = DepthInfo::NO_CONFIDENCE;
