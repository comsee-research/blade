	...
	else if (dm.is_refined_map())
	{
		...
		else
		{
			...
			for(std::size_t k = kmin; k < kmax; ++k)
			{
				...
				{
					//get pixels
				 	const std::vector<IndexPair> pixels = pixels_neighbors(mfpc.mia(), dm.width(), dm.height(), k, l); 
					...
					{		
						...
						//get neighbors
					 	std::vector<IndexPair> neighs = neighbors(mfpc.mia(), k, l, d, d); 	
					 	std::vector<double> depths; depths.reserve(neighs.size() + 1);
	...
	if (dm.is_coarse_map())
	{
		...
		for(std::size_t k = kmin; k < kmax; ++k)
		{
			for(std::size_t l = lmin; l < lmax; ++l)
			{
				...
				//get neighbors
			 	std::vector<IndexPair> neighs = neighbors(mia, k, l, sd, sd);  
		...
		for(std::size_t k = kmin; k < kmax; ++k)
		{
			...
			{				
				if (permicroimage)
				{
					...
					//get neighbors
				 	std::vector<IndexPair> neighs = pixels_neighbors(mia, dm.width(), dm.height(), k, l); 
	...
	if (dm.is_coarse_map())
	{
		...
		for(std::size_t k = kmin; k < kmax; ++k)
		{
			for(std::size_t l = lmin; l < lmax; ++l)
			{
				...
				//get neighbors
			 	std::vector<IndexPair> neighs = neighbors(mfpc.mia(), k, l, sz, sz); 
